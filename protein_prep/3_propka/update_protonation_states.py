...
def edit_pdb_file(
    pdb_file: str, target_res_name: str, target_res_numb: str
) -> List[list]:
    ...
    with open(pdb_file) as file_in:
        for line in file_in:
            ...
            if "ATOM" in line:
                ...
                if (line_info[3] == target_res_name) and (
                    line_info[5] == target_res_numb
                ):
                    new_line = line.replace(
                        target_res_name, ALLOWED_CHANGES[target_res_name]
                    )
                    exchange_made = True
                    if (line_info[3] == "LYS") and (line_info[2] == "HZ1"):
                        continue
                    else:
    ...
    if not exchange_made:
        print(
            f"""Warning, no change was made for the file and params:
...
def main():
    for fix_item in FIXES_REQUIRED:
        ...
        new_pdb_file = edit_pdb_file(
            pdb_file=file_path, target_res_name=fix_item[1], target_res_numb=fix_item[2]
        )
